...
class Player(db.Model):
    ...
    def can_set_free_custom_role(self):
        """Check if player can set free custom role (reputation 500+)"""
        return self.reputation >= 500

    @property
    def display_role(self):
        """Get the role to display (custom role if available, otherwise regular role)"""
        if hasattr(self, 'custom_role_purchased') and self.custom_role_purchased and self.custom_role:
...
class Quest(db.Model):
    ...
    quest_category = db.Column(db.String(20), default='permanent')  # daily, weekly, monthly, thematic, mythic, permanent
    ...
    def refresh_timed_quests(cls):
        ...
        # Check and refresh daily quests (24 hours)
        daily_quests = cls.query.filter_by(quest_category='daily', is_active=True).all()
        for quest in daily_quests:
            if quest.last_refresh and (current_time - quest.last_refresh).total_seconds() >= 86400:  # 24 hours
        ...
        # Check and refresh weekly quests (168 hours)
        weekly_quests = cls.query.filter_by(quest_category='weekly', is_active=True).all()
        for quest in weekly_quests:
            if quest.last_refresh and (current_time - quest.last_refresh).total_seconds() >= 604800:  # 168 hours
                quest.last_refresh = current_time
                PlayerQuest.query.filter_by(quest_id=quest.id).update({
                    'is_completed': False,
                    'is_accepted': False,
        ...
        # Check and refresh monthly quests (720 hours)
        monthly_quests = cls.query.filter_by(quest_category='monthly', is_active=True).all()
        for quest in monthly_quests:
            if quest.last_refresh and (current_time - quest.last_refresh).total_seconds() >= 2592000:  # 720 hours
                quest.last_refresh = current_time
                PlayerQuest.query.filter_by(quest_id=quest.id).update({
                    'is_completed': False,
                    'is_accepted': False,
...
class ShopItem(db.Model):
    ...
    description = db.Column(db.Text, nullable=False)
    category = db.Column(db.String(50), nullable=False)  # title, booster, theme, gradient, avatar, cursor
    ...
    def apply_item_effect(self, player):
        ...
        try:
            ...
            if self.category == 'custom_role':
                ...
                # Create custom title
                title = CustomTitle(
                    title=data.get('title_text', self.display_name),
                    color=data.get('title_color', '#ffd700'),
                    is_gradient=data.get('is_gradient', False),
                ...
                # Apply temporary booster effects
                booster_type = data.get('booster_type', 'xp')
                multiplier = data.get('multiplier', 2.0)
                duration_hours = data.get('duration_hours', 24)

                # Implementation depends on booster system
                # For now, just give immediate bonus
                if booster_type == 'xp':
...
class ShopPurchase(db.Model):
    ...
    purchase_price_reputation = db.Column(db.Integer, default=0, nullable=False)
...
class Achievement(db.Model):
    ...
    def create_default_achievements(cls):
        ...
        default_achievements = [
            ...
            # Epic achievements (high rewards)
            {
                'title': 'Неудержимый',
            ...
            # Legendary achievements (very high rewards)
            {
            ...
            {
                'title': 'Великий воин',
                'description': 'Убейте 500 игроков',
                'icon': 'fas fa-shield',
                'rarity': 'legendary',
                'unlock_condition': '{"kills": 500}',
                'reward_xp': 10000,
                'reward_coins': 2000,
                'reward_reputation': 100,
            ...
            {
                'title': 'Легенда арены',
                'description': 'Выиграйте 100 игр с 85%+ винрейтом',
                'icon': 'fas fa-medal',
                'rarity': 'legendary',
                'unlock_condition': '{"wins": 100, "win_rate": 85.0}',
                'reward_xp': 12000,
                'reward_coins': 2500,
                'reward_reputation': 120,
